...
;
; Syntax      :	STEREO_IGRF_POLE, DATE, THETA, PHI
;
; Examples    :	STEREO_IGRF_POLE, '2007-01-31', THETA, PHI
;
...
;
;                               ERRMSG = ''
;                               STEREO_IGRF_POLE, DT, LN, LT, ERRMSG=ERRMSG
;                               IF ERRMSG NE '' THEN ...
;
; Calls       :	CONCAT_DIR, STR_SEP, ANYTIM2UTC, UTC2TAI, INTERPOL
;
; Common      :	Common block STEREO_IGRF_POLE contains the coefficients 
;
; Restrictions:	This version of the routine is written against IGRF-11, and is
;               valid between 1900 and 2015.  When future model versions are
...
;               Version 4, 15-Apr-2016, William Thompson, GSFC
;                       Use IGRF-12
;
; Contact     :	WTHOMPSON
;-
...
;
on_error, 2
common stereo_igrf_pole, years, g10coeff, g11coeff, h11coeff
;
;  Check the input parameters.
;
if n_params() ne 3 then begin
    message = 'Syntax: STEREO_IGRF_POLE, DATE, THETA, PHI'
    goto, handle_error
endif
;
...
if n_elements(years) eq 0 then begin
    file = concat_dir(getenv('STEREO_SPICE_GEN'), 'igrf12coeffs.txt')
...
if n_elements(errmsg) eq 0 then message, message else $
  errmsg = 'stereo_igrf_pole: ' + message
;
end
