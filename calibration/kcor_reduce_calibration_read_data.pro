pro kcor_reduce_calibration_read_data, file_list, data, metadata, verbose=verbose
	...
	for f = 0, n_elements(file_list)-1 do begin
		...
		if strmatch(header.darkshut, '*in*', /fold_case) then begin
			...
			if strmatch(header.calpol, '*out*', /fold_case) then begin
				...
				if keyword_set(verbose) then print, 'Found calibration data, file ' + file_list[f] + ', angle ' + string(header.calpang)
			endelse
		endif
	endfor
	...
	metadata = { angles:angles, idiff:idiff, vdimref:vdimref, date:date, file_list:file_list, file_types:file_types }
end
