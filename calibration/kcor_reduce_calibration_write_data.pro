pro kcor_reduce_calibration_write_data, data, metadata, $
	...
	pixelsvar = ncdf_vardef(cid, 'Pixels Fit with Model', [vectordim,pixelsdim], /short)
	fitsvar = ncdf_vardef(cid, 'Model Fit Parameters', [paramsdim,pixelsdim], /float)
	fiterrorsvar = ncdf_vardef(cid, 'Model Fit Parameters Formal Errors', [paramsdim,pixelsdim], /float)
	...
	if filelistvar eq -1 or filetypesvar eq -1 or darkvar eq -1 or gainvar eq -1 or $
		dimrefvar eq -1 or fitsvar eq -1 or fiterrorsvar eq -1 or $
