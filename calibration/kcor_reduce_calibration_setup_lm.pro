pro kcor_reduce_calibration_setup_lm, data, metadata, pixel, beam, parinfo, functargs
	...
	if beam eq 0 then parinfo[9:12].value = [!pi,3*!pi/2,!pi/2,0] ; this should be really close
	if beam eq 1 then parinfo[9:12].value = [0,!pi/2,3*!pi/2,!pi] ; this should be really close
