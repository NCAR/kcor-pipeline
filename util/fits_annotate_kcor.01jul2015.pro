...
PRO fits_annotate_kcor, hdu, xdim, ydim, xb, yb, dmin, dmax, dexp
...
;  print, 'cs1/2/3/4: ', cs1, cs2, cs3, cs4
...
;  time_obs = fxpar (hdu, 'TIME-OBS')
  ...
  date_obs = fxpar (hdu, 'DATE-OBS')		; 'yyyy-mm-ddThh:mm:ss'
  object   = fxpar (hdu, 'OBJECT')
  type_obs = fxpar (hdu, 'DATATYPE')
...
;  dispmax   = fxpar (hdu, 'DISPMAX')
  ...
  rsun     = fxpar (hdu, 'RSUN')
  dataform = 'pB pseudo calibrated'

;  dataform = fxpar (hdu, 'DATAFORM')

  inst_rot    = fxpar (hdu, 'INST_ROT')			; instrument roll angle
  solar_p0  = fxpar (hdu, 'SOLAR_P0')			; P-angle
;  solar_b0  = fxpar (hdu, 'SOLAR_B0')			 ; B-angle
  solar_b0  = fxpar (hdu, 'CRLT_OBS')			; B-angle
  solar_l0  = fxpar (hdu, 'CRLN_OBS')			; L-angle
  car_rot   = fxpar (hdu, 'CAR_ROT')			; Carrington rotation

  wavelnth  = fxpar (hdu, 'WAVELNTH')			; wavelength
  wavefwhm  = fxpar (hdu, 'WAVEFWHM')			; wavelength FWHM

  expdur    = fxpar (hdu, 'EXPTIME')

;--- SMM C/P fits keywords :
;
;  colorfil  = fxpar (hdu, 'COLORFIL')
;  glfilter  = fxpar (hdu, 'GLFILTER')
;  polaroid  = fxpar (hdu, 'POLAROID')
;  sector    = fxpar (hdu, 'SECTOR')
;  calmir    = fxpar (hdu, 'CALMIR')
;  expdur    = fxpar (hdu, 'EXPDUR')
...
;    sdmin    = STRTRIM (STRING (dmin,    FORMAT='(E9.3)'), 2)
;  IF (dmin LT 10000.0 AND dmin GT 1.0) THEN		$
;    sdmin    = STRTRIM (STRING (dmin,    FORMAT='(F9.3)'), 2)
;  IF (dmin EQ 0.0) THEN		$
;    sdmin    = STRTRIM (STRING (dmin,    FORMAT='(I9)'  ), 2)
  sdmin = STRTRIM (STRING (dmin, FORMAT='(F9.3)'), 2)

  IF (dmax GE 10000.0 OR dmax LT 1.0) THEN		$
  BEGIN
;    print,'>10k dmax: ', dmax
    sdmax    = STRTRIM (STRING (dmax,    FORMAT='(E9.3)'), 2)
  END

  if (dmax LT 10000.0 AND dmax GT 1.0) THEN		$
  BEGIN
;    print,'<10k dmax: ', dmax
    sdmax    = STRTRIM (STRING (dmax,    FORMAT='(F9.3)'), 2)
  END
...
;    print,'zero dmax: ', dmax
  ...
  sroll   = STRTRIM (STRING (roll,    FORMAT='(F5.1)'), 2)
...
;  print, 'object: ', object
;
  IF (object EQ 'CORONA' OR object EQ 'Solar K-Corona') THEN	$
    pixval = 'K-Corona'			$
...
;  	font=lfont, charsize=cs1, color=black
  ...
  title  = telescop + '    Mauna Loa Solar Observatory   HAO/NCAR'
  lentit = STRLEN (title)
;  print, 'lentit: ', lentit
;  x4 = 12
;  print, 'x4: ', x4
