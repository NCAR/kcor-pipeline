;+
; NAME		suncir_comp.pro
;
; PURPOSE	Draw a circle at 1.0 Rsun.
;
; SYNTAX	suncir_comp, xdim, ydim, xcen, ycen, xb, yb, pixrs, roll
;
...
; 29 Jun 2015 Adapt for comp.
;-

PRO suncir_comp, xdim, ydim, xcen, ycen, xb, yb, pixrs, roll
   ...
   xg = FIX (xcen + xb + 0.5)
   yg = FIX (ycen + yb + 0.5)
   rp = FIX (pixrs / 10.0 + 0.5) - 1	; ~ 0.1 Rsun
;   rp = rp * (pixrs / 160.0)
   ...
   BEGIN ;{
      ...
      PLOTS, xg-rp,   yg,    /device, color=grey
      PLOTS, xg,      yg+rp, /device, color=grey, /continue

      PLOTS, xg+1,    yg+rp, /device, color=grey
      PLOTS, xg+rp+1, yg,    /device, color=grey, /continue

      PLOTS, xg-rp,   yg,    /device, color=grey
      PLOTS, xg-1,    yg,    /device, color=grey, /continue

      PLOTS, xg+2,    yg,    /device, color=grey
      PLOTS, xg+rp+1, yg,    /device, color=grey, /continue
      ...
      PLOTS, xg,    yg-rp, /device, color=grey
      PLOTS, xg,    yg-1,  /device, color=grey, /continue

      PLOTS, xg+1,  yg-rp, /device, color=grey
      PLOTS, xg+1,  yg-1,  /device, color=grey, /continue

      PLOTS, xg,    yg+2,  /device, color=grey
      PLOTS, xg+1,  yg+2,  /device, color=grey, /continue
      END   ;}
...
;   tvcircle, r3, xcen, ycen, color=254, /device
   ...
   ;-----------------------------------------------------------
   ; Draw a circle at 1.0 Rsun:  one BOLD dot every 30 degrees.
   ;-----------------------------------------------------------
...
;      PLOTS, xg,   yg,   /device, color=white, /continue
      ...
      PLOTS, xg-3, yg-3, /device, color=blue
      PLOTS, xg+3, yg-3, /device, color=blue, /continue
      PLOTS, xg+3, yg+3, /device, color=blue, /continue
      PLOTS, xg-3, yg+3, /device, color=blue, /continue
      PLOTS, xg-3, yg-3, /device, color=blue, /continue
;      PLOTS, xg+3, yg+3, /device, color=blue, /continue
   ...
   ;-----------------------------------------------------------
   ; Draw a circle at 1.0 Rsun:  one BOLD dot every 90 degrees.
   ;-----------------------------------------------------------
...
;      PLOTS, xg,   yg,   /device, color=white
;      PLOTS, xg,   yg,   /device, color=white, /continue
      PLOTS, xg-3, yg-3, /device, color=green
      PLOTS, xg+3, yg-3, /device, color=green, /continue
      PLOTS, xg+3, yg+3, /device, color=green, /continue
      PLOTS, xg-3, yg+3, /device, color=green, /continue
      PLOTS, xg-3, yg-3, /device, color=green, /continue
   END   ;}
   ...
   ;------------------------------------------------------------
   ; Draw a circle at 1.35 Rsun:  one BOLD dot every 30 degrees.
   ;------------------------------------------------------------
   ...
   BEGIN ;{
      ...
      PLOTS, xg,   yg,   /device, color=white
      PLOTS, xg,   yg,   /device, color=white, /continue
      PLOTS, xg-2, yg-2, /device, color=blue
      PLOTS, xg+2, yg-2, /device, color=blue, /continue
      PLOTS, xg+2, yg+2, /device, color=blue, /continue
      PLOTS, xg-2, yg+2, /device, color=blue, /continue
      PLOTS, xg-2, yg-2, /device, color=blue, /continue
   END   ;}
   ...
   ;------------------------------------------------------------
   ; Draw a circle at 1.35 Rsun:  one BOLD dot every 90 degrees.
   ;------------------------------------------------------------
   ...
   BEGIN ;{
      ...
      PLOTS, xg,   yg,   /device, color=white
      PLOTS, xg,   yg,   /device, color=white, /continue
      PLOTS, xg-2, yg-2, /device, color=green
      PLOTS, xg+2, yg-2, /device, color=green, /continue
      PLOTS, xg+2, yg+2, /device, color=green, /continue
      PLOTS, xg-2, yg+2, /device, color=green, /continue
      PLOTS, xg-2, yg-2, /device, color=green, /continue
   END   ;}
