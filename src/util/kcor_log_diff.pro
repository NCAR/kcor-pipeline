...
;-
function kcor_log_diff_read, filename, count=count
  ...
  if (tarlist) then begin
    ...
    for i = 0L, count - 1L do begin
      ...
      if (n_tokens ge 6) then begin
        dts[i] = file_basename(tokens[5], '_kcor.fts.gz')
      endif else begin
        dts[i] = lines[i]
      endelse
    endfor
...
;     `filename2` log
;-
function kcor_log_diff, filename1, filename2, $
                        only_file1=only_file1, only_file2=only_file2
  compile_opt strictarr

  diff = 0B

  dt1 = kcor_log_diff_read(filename1, count=n_files1)
  dt2 = kcor_log_diff_read(filename2, count=n_files2)
...
dir = '/Data/KCor/raw/20181115/level0'
filename1 = filepath('20181115.kcor.machine.log', root=dir)
filename2 = filepath('20181115.kcor.t1.log', root=dir)
;filename2 = filepath('20181115.kcor.t2.log', root=dir)
;filename2 = filepath('20181115_kcor_l0.tarlist', root=dir)

diff = kcor_log_diff(filename1, filename2, only_file1=only_file1, only_file2=only_file2)
