...
function kcor_verify_dates_expandrange, start_date, end_date, count=n_days
  compile_opt strictarr

  start_parts = kcor_decompose_date(start_date)
  start_jd = julday(start_parts[1], start_parts[2], start_parts[0], 0, 0, 0.0D)
  end_parts = kcor_decompose_date(end_date)
...
pro kcor_verify_dates, date_expression, config_filename=config_filename
  compile_opt strictarr
  on_error, 2
  ...
  for r = 0L, n_ranges - 1L do begin
    ...
    case n_endpts of
      ...
      2: begin
          dates = kcor_verify_dates_expandrange(endpts[0], endpts[1], $
                                                count=n_dates)
  ...
  if (n_failed_days gt 0L) then begin
    ...
    mg_log, 'no failed days', name='kcor/verify', /info
  endelse
