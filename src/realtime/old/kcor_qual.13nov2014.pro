...
IF (np EQ 0) THEN $
BEGIN ;{
   PRINT, "kcor_gbu, 'yyyymmdd', list='list'"
   RETURN
END   ;}
...
PRINTF, ULOG, 'q_dir_sobel:  ', q_dir_sobel
...
BEGIN ;{
   ...
   ;--- Define variables for circle "scans".
...
;      PRINT,        'datatype:  ', datatype
   ...
   radius_pix = 0.0
   IF (cal GT 0 OR cloudy GT 0) THEN $	; Use fixed location for center.
   BEGIN
      xcen = axcen - 4
      ycen = aycen - 4
   END  $
   ...
   BEGIN ;{
      center_info = find_image (img00,chisq=chisq,radius_guess=175.0,drad=30.0)
      xcen = axcen + center_info (0)		; x offset
      ycen = aycen + center_info (1)		; y offset
...
;   if ((chksobel GT 0) AND (bad EQ 0)) THEN $
   BEGIN ;{
      PRINT, '~ ~ ~ Sobel check ~ ~ ~'
      nray  = 480
      acirc = !PI * 2.0 / float (nray)
...
;   imax = 3000 ^ power
   ...
   pb0sb = bytscl (pb0s, min=imin, max=imax)
