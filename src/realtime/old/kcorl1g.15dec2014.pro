...
pro kcorl1g, date_str, list=list, base_dir=base_dir
...
; set zero and negative values in gain to value in gain_negative
    gain_negative = -10
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT: this is needed to compute the radius of the Sun 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT : FIND IMAGE CENTER
 
;  FIND CENTER AND RADIUS FOR CAMERA 0
    
      center_info_img  = kcor_find_image (img (*, *, 0, 0),  radius_guess, /center_guess)
...
;   occulter in 
    ...
    replace = where(rr0 gt radius_0 -1. and grr0 le center_info_gain0(2) +2. , nrep)
    gain_temp =  gain_alfred (*, *, 0)
    gain_replace =   shift (gain_alfred (*, *, 0), xctr0 - center_info_gain0(0), yctr0 - center_info_gain0(1) )
    ...
    gain_replace =  shift (gain_alfred (*, *, 1), xctr1 - center_info_gain1(0), yctr1 - center_info_gain1(1) )
...
; GdT : DO NOT NEED TO CENTER IMAGE AGAIN AFTER DEMODULATION
;       BUT  NEED TO COMPUTE NEW CENTER  AFTER DISTORTION TO APPLY TO THE END 
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT : This  is all replaced by array operation 
;
...
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;   GdT: I think these phases should be zero. I leave them for now 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT: this is replaced to avoid loop 
;
;
;   angle0   = fltarr (360)
;   angle1   = fltarr (360)
...
;  THE EQUATIONS ARE NOT CORRECT. I LEFT THEM THE SAME FOR NOW
;  should be  by 45deg. i.e. 90deg in sin2theta
...
;  make new theta arrays in degrees
   ...
   angle=float(i)
   pick0 = where(rr0 ge r_in and rr0 le r_out and theta0_deg ge angle and theta0_deg lt angle +1., nnl0)
   if nnl0 gt 0 then angle_ave_u0(i) = mean( umk4_0(pick0))
   pick1 = where(rr1 ge r_in and rr1 le r_out and theta1_deg ge angle and theta1_deg lt angle +1., nnl1) 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT: left 0.5 to avoid going negabtive
;      this should be 1.0
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT:  update center info after distrosion  
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT: SHOULD BE CHANGED TO USE BILINEAR INTERPOLATION
...
;      FOR s = 0, 2 do begin
;         cal_data (*, *, 0, s) = fshift (cal_data (*, *, 0, s), 511.5 - xctr0, 511.5 - yctr0)
;         cal_data (*, *, 1, s) = fshift (cal_data (*, *, 1, s), 511.5 - xctr1, 511.5 - yctr1)
;      ENDFOR

      FOR s = 0, 2 do begin
         cal_data (*, *, 0, s) = rot (reverse(cal_data (*, *, 0, s),1) , pangle, 1, xsize-1-xctr0, yctr0, cubic=-0.5)
         cal_data (*, *, 1, s) = rot (reverse(cal_data (*, *, 1, s),1) , pangle, 1, xsize-1-xctr1, yctr1, cubic=-0.5)
      ENDFOR
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT: Changed overocculting from 5 pixels to 2 pixels
; if this does not work, we can go back to 5
...
; GdT: USE MASK TO BUILT FINAL IMAGE
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT: This is not necessary, it was done at the beginning
;
;   ;--- CDS format?
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT:  Nothng was changed after this point
