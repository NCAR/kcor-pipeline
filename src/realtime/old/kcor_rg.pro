pro kcor_rg, fits_file, fits=fits
;+
;-------------------------------------------------------------------------------
; NAME:
;   kcor_rg
;
; PURPOSE:
;   Apply Radially-Graded filter to kcor a image and write to a GIF file.
;
; INPUTS:
;   fits_file:	kcor L1 fits file
;
; OPTIONAL keyword:
;   /fits	if specified, also write RG image to a FITS file.
;
; OUTPUTS:
;   gif file
;   fits file	(if /fits keyword is used)
;
; AUTHOR:
...
; 26 Jan 2016 Add "rg" instead of "nrg" to output file name.
; 03 Mar 2016 Change RG FITS file to short integer (instead of float).
; 04 Mar 2016 Change output gif file name back to yyyymmdd_hhmmss_kcor_nrgf.gif
;             Change output fts file name back to yyyymmdd_hhmmss_kcor_nrgf.fts
;-------------------------------------------------------------------------------
;-
...
begin ;{
  ...
  rg_file = strmid (fits_file, 0, fts_loc) + '_nrgf.fts'
