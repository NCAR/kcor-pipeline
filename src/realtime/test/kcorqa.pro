...
;-------------------------------------------------------------------------------
; :Params: 
; img			Kcor Level 0 raw pB image [1024x1024]
; hdu			FITS header.
; /gif			if set, display gif file.
; /debug		if set, print information.
;
; sample calls:
; iq = kcorqa (img, hdu)
; iq = kcorqa (img, hdu, /debug)
;-------------------------------------------------------------------------------;- 
FUNCTION kcorqa, img, hdu, l0_file, gif=gif, debug=debug

iq = 'u'

;--- Define mask file name.  
...
;------------------------------------------------------------------------------
   ...
   IF (KEYWORD_SET (debug)) THEN $
      PRINT,        '>>>>>>> ', l0_file, '   ', datatype, ' <<<<<<<'
;   PRINTF, ULOG, '>>>>>>> ', l0_file, '   ', datatype, ' <<<<<<<'
...
;   PRINT,        'nelem:     ', nelem
   ...
   iaxcen = fix (axcen + 0.5)
   iaycen = fix (aycen + 0.5)
...
;   print, '+++  4 +++'
   ...
   radius_pix = 0.0
   IF (nok GT 0) THEN $	; Use fixed location for center.
   BEGIN
      xcen = axcen - 4
      ycen = aycen - 4
   END  $
   ...
   BEGIN ;{
      ...
      radius_pix = center_info (2)		; radius
...
;   PRINT,        'xcen,ycen,rpix:      ', xcen, ycen, radius_pix
;   PRINT,        'ixcen,iycen: ', ixcen, iycen
;   PRINTF, ULOG, 'ixcen,iycen: ', ixcen, iycen
;   PRINTF, ULOG, 'xcen,ycen,rpix:      ', xcen, ycen, radius_pix

;   print, '+++  5 +++'

   ;--- Rotate image by P-angle.
   ;    Rotation moves [xcen, ycen] to array center [511.5, 511.5].
...
;   print, '+++  7 +++'
   ...
   BEGIN ;{
      ...
      rpixs = 215			; circle radius [pixels].
      dpx   = fix (cos (dp) * rpixs + axcen + 0.5005)
      dpy   = fix (sin (dp) * rpixs + aycen + 0.5005)
...
;          PRINT, 'saturation check ssize:  ', ssize
	 ...
	 BEGIN ;{
	    ...
	    BEGIN ;{
               ...
               PRINT,        'saturation radius, limit: ', rpixs, smax
               PRINT,        'saturation image info: '
               PRINT,        pb0rot (dpx, dpy)
               PRINT,        'saturation average:   ', satave
;               PRINTF, ULOG, '~ ~ ~ Saturation check ~ ~ ~'
;               PRINTF, ULOG, 'saturation radius, limit: ', rpixs, smax
;               PRINTF, ULOG, 'saturation image info: '
;               PRINTF, ULOG, pb0rot (dpx, dpy)
;               PRINTF, ULOG, 'saturation average:   ', satave
	    END   ;}
	    sat = 1
;            PRINT,        '*** ', rpixs, ' ring saturated:', nelem
;            PRINTF, ULOG, '*** ', rpixs, ' ring saturated:', nelem
	 END   ;}
      END   ;}
   END   ;}
...
;   print, '+++  9 +++'
   ...
   ;----------------------------------------------------------------------------
   ;--- Do sobel test for "good" images.
   ;----------------------------------------------------------------------------

   chksobel = 1
   sobel    = 0
   bad = bright + sat + clo + chi + dev
   if ((chksobel GT 0) AND (bad EQ 0) AND (cal EQ 0)) THEN $
   BEGIN ;{
      nray  = 480
      acirc = !PI * 2.0 / float (nray)
      dp    = findgen (nray) * acirc
...
;      sobel_diff_limit = 15.0
      sobel_diff_limit = 50.0		; difference threshold.
;      sobel_diff_limit = 70.0		; difference threshold.
      ...
      rpixbeg = 276
      rpixend = 280
      FOR rpixd = rpixbeg, rpixend, 1 do $
      BEGIN ;{
         dpx = fix (cos (dp) * rpixd + axcen + 0.5005)
	 dpy = fix (sin (dp) * rpixd + aycen + 0.5005)
	 ksobel = float (pb0rot (dpx, dpy))
	 kdiff  = abs (ksobel (0:nray-2) - ksobel (1:nray-1))
	 badpix = where (kdiff GT sobel_diff_limit)
	 ...
	 BEGIN ;{
	    ...
	    !p.multi = [0, 1, 2]
;            plot, ksobel, title = l0_file + ' sobel'
;            plot, kdiff,  title = l0_file + ' diff sobel'
	 ...
	 ;--- The sobel itself is not a reliable test.
	 ;    The difference of the sobel works well.
...
;            PRINT,        'rpixd: ', rpixd, ' sobel diff:'
;            PRINT,        fix (kdiff (badpix))
;            PRINT,        'sobel diff numbad: ', numbad

;	    PRINTF, ULOG, 'rpixd: ', rpixd, ' sobel diff: '
;	    PRINTF, ULOG, fix (kdiff (badpix))
;	    PRINTF, ULOG, 'sobel diff numbad: ', numbad

	 END   ;}
      END   ;}

      ;--- If sobel limit is exceeded, set bad = 1.
	 ...
	 BEGIN ;{
            PRINT, '~ ~ ~ Sobel check ~ ~ ~'
            PRINT,        'sobel_diff_limit, total_bad_limit: ', $
                           sobel_diff_limit, total_bad_limit

            PRINT,        'sobel total bad: ', total_bad
            PRINT,        'sobel bad limit: ', total_bad_limit
            PRINT,        '*** sobel limit exceeded.'

;            PRINTF, ULOG, '~ ~ ~ Sobel check ~ ~ ~'
;            PRINTF, ULOG, 'sobel_diff_limit, total_bad_limit: ', $
;                          sobel_diff_limit, total_bad_limit
;            PRINTF, ULOG, 'sobel total bad: ', total_bad
;            PRINTF, ULOG, 'sobel bad limit: ', total_bad_limit
;	     PRINTF, ULOG, '*** sobel limit exceeded.'
	 END   ;}
      END   ;}
   END   ;}
...
;   print, '+++ 11 +++'
   ...
   power = 0.8
   power = 0.5
   pb0s = pb0m ^ power			; intensity scaling by exponential power
...
;   print, '+++ 12 +++'
   ...
   PRINT, 'axcen, aycen: ', axcen, aycen
;   PRINT, '!D.N_COLORS: ', !D.N_COLORS

   irsunpix = FIX ((rsun / 5.643) + 0.5)	; 1.0 Rsun [pixels]
   ...
   BEGIN
      ...
      tvcircle, irsunpix, axcen, aycen,   0, /device, /fill   ; 1.0 Rsun circle
      tvcircle, irsunpix, axcen, aycen, red, /device          ; 1.0 Rsun circle
      tvcircle, irsunpix*3.0, axcen, aycen, grey,   /device   ; 3.0 Rsun circle
;      tvcircle, rpixb,        axcen, aycen, red,    /device   ; bright   circle
;      tvcircle, rpixc,        axcen, aycen, green,  /device   ; cloud    circle
;      tvcircle, rpixs,        axcen, aycen, blue,   /device   ; sat      circle
;      tvcircle, rpixd,        axcen, aycen, yellow, /device   ; sobel    circle

      ;--- Draw "+" at sun center.

      plots, [iaxcen-5, iaxcen+5], [iaycen, iaycen], color=yellow, /device
      plots, [iaxcen, iaxcen], [iaycen-5, iaycen+5], color=yellow, /device
...
;   print, '+++ 13 +++'
   ...
   ;--- Create default GIF file name.

   fitsloc = STRPOS (l0_file, '.fts')
   gif_file = 'kcor.gif'			; default gif file name.
   gif_file = strmid (l0_file, 0, fitsloc) + '.gif' 

   xyouts, 4, ydim-20, gif_file, color=red, charsize=1.5, /device
;   save = tvrd ()

   ;----------------------------------------------------------------------------
   ;--- Create GIF file name, set quality designation, draw circle (as needed).
   ;----------------------------------------------------------------------------

   iq = 'u'				; default quality: u = unknown.
...
;      gif_path = q_dir_sat   + gif_file
   ...
   ELSE  $
   IF (sobel  GT 0) THEN $		; Sobel difference.
   BEGIN   ;{
      tvcircle, rpixd,        axcen, aycen, yellow, /device   ; sobel    circle
;      gif_file = strmid (l0_file, 0, fitsloc) + '_s.gif' 
;      gif_path = q_dir_sobel + gif_file
      iq = 's'
   END   $ ;}
...
;PRINT,         'end of image loop'
;PRINTF, ULIST, 'end of image loop'
