PRO  kcor_comb, file=file, cunit=cunit, plot=plot
...
;       plot, gain_temp (*, 1000)
       filter = mean_filter (gain_temp, 5, 5, invalid = gain_negative, $
                             missing=1)
...
;         wait, 1 
   ...
   if (plot eq 1) then begin
      ...
      loadct, 39
      draw_circle, center0_info_gain (0), center0_info_gain (1), $
                   center0_info_gain (2), /dev, color= 250    
      ...
      loadct, 39
      draw_circle,  center1_info_gain (0), center1_info_gain (1), $
                    center1_info_gain (2), /dev, color= 250    
      loadct, 0
   ...
   ;----------------------------------------------------------------------------
   ; Define coordinate arrays for gain.
   ;----------------------------------------------------------------------------

   gxx0 = findgen (xsize, ysize) mod (xsize) - center0_info_gain (0)
   gyy0 = transpose (findgen (ysize, xsize) mod (ysize) ) $
        - center0_info_gain (1)
   ...
   gyy1 = transpose (findgen (ysize, xsize) mod (ysize) ) $
        - center1_info_gain (1)
...
;   gainshift = fltarr (xsize,ysize, 2)
   ...
   ;----------------------------------------------------------------------------
   ; Find centers of raw L0 images.
   ;----------------------------------------------------------------------------

   ;--- Find center for camera 0.
    
   center0_info_img  = kcor_find_image (img (*, *, 0, 0), $
...
;   for s = 0,3 do img(*,*, s, 0) =  img(*,*, s,0)*mask_occulter0
   ...
   center1_info_img  = kcor_find_image (img (*, *, 0, 1), $
                                        radius_guess, /center_guess)
...
;    replace = where(rr0 gt radius_0 -1. and $
;                   grr0 le center0_info_gain(2) +2. , nrep)
;    if nrep gt 0 then gain0 (replace) = 1350. ; gain_no_occulter (replace)
;    replace = where(rr1 gt radius_1 -1. and $
;                   grr1 le center1_info_gain (2) +2. , nrep)
...
;       tv, bytscl (gain_alfred (*,*,1), 0, 3000)
   ...
   ; Replace data where we do not have the gain, i.e. pixels under the occulter
   ; in the gain, with gainshifted --- we will use the reference gain 
   ; with no occulter in the future.

   replace = where (rr0 gt radius_0 - 3. $
                    and grr0 le center0_info_gain (2) + 3.0, nrep)
   ...
   replace = where (rr1 gt radius_1 - 3. $
                    and grr1 le center1_info_gain (2) + 3.0, nrep)
   if (nrep gt 0) then $
   begin 
      gain_temp    =  gain_alfred (*, *, 1)
      gain_replace = shift (gain_alfred (*, *, 1), xctr1 $
                   - center1_info_gain (0), yctr1 - center1_info_gain (1) )
   ...
   for b = 0, 1 do begin     
      for y = 0, 1023 do begin
         for x = 0, 1023 do begin
            cal_data (x,y,b,*) = reform (dmat (x, y, b, *, *)) $
	                      ## reform (img_new (x, y, *, b))
         endfor
      endfor
   endfor   
   ...
   ;----------------------------------------------------------------------------
   ; Find center for images after distortion correction.
   ;----------------------------------------------------------------------------
   ...
   ;----------------------------------------------------------------------------
   ; This shifts the images in the middle of the array and puts north up.
   ...
   if (shift_center eq 1) then begin
      ...
      FOR s = 0, 2 DO BEGIN 
         ...
         cal_data_combined_center (*, *, s) = $
            (cal_data_new (*, *, 0, s) + cal_data_new (*, *, 1, s)) * 0.5
      ENDFOR 
...
;   a(1) = 0.25
   ...
   BEGIN ;{
      ...
      r_in  = r_in  * radsun * 60.0 / 5.643
      r_out = r_out * radsun * 60.0 / 5.643
...
;   print, 'mean (umk4_int): ', mean (umk4_int)
   print, 'mean (umk4), mean (intensity): ', mean (umk4), mean (intensity)
	 ...
	 BEGIN ;{
      ...
      begin ;{
         ...
         oplot, degrees * !radeg, $
	    a(0) * sin (2.0*degrees -90. / !radeg + a(1)) , color=100, thick=5
         oplot, degrees * !radeg, $
...
;   radial_amplitude1 = interpol(amplitude1, radscan, rr1, /quadratic)
   ...
   if (plot eq 1) then begin 
      plot, rr1 (*, 500) * 5.643 / (radsun * 60.), radial_amplitude1 (*, 500), $
            xtitle='distance (solar radii)', $
            ytitle='amplitude', title='CAMERA 1'
      oplot, radscan * 5.643 / (radsun * 60.0), amplitude1, psym=2
      wait,1
   endif
   ...
   sky_polar_q1 = radial_amplitude1 $
                * sin (2.0 * theta1 - 90.0 / !radeg + mean_phase1 ) - bias
   ...
   begin 
      ...
      for ii = 0,numdeg - 1 do plots, $
         [xctr1, xctr1 + 500 * cos (degrees (ii))], $
	 [yctr1, yctr1 + 500 * sin (degrees (ii))], /dev
      wait,1
   endif
...
;   print,'pausing'
   ...
   r_in = fix (occulter / platescale) + 2.0
   r_out = 504.0
