...
; 20 Mar 2015 [ALS] Combine images from both cameras prior to removal
...
;  fit in U/I and Q/I
   ...
   BEGIN ;{
      ...
      r_in  = r_in  * radsun * 60.0 / platescale
      r_out = r_out * radsun * 60.0 / platescale
...
;   radial_amplitude1 = interpol (amplitude1, radscan, rr1, /quadratic)
   ...
   IF (doplot EQ 1) THEN $
   BEGIN ;{
      plot, rr1 (*, 500) * platescale / (radsun * 60.0), $
            radial_amplitude1 (*, 500), $
	    xtitle='distance (solar radii)', $
            ytitle='amplitude', title='CAMERA 1'
      oplot, radscan * platescale / (radsun * 60.0), amplitude1, psym=2
      wait,1
   ENDIF ;}
...
;                        ' calibration file:dark, opal, 4 pol.states'
   ...
   sxaddpar, newheader, 'L1SWID',   'kcorl1s.pro 19mar2015', $
                        ' Level 1 software'
   ...
   sxaddpar, newheader, 'SGSRAV',   struct.sgsrav, $
                        ' [V] mean SGS RA error signal', format = '(e11.4)'
   ...
   sxaddpar, newheader, 'SGSDECV',  struct.sgsdecv, $
                        ' [V] mean SGS DEC error signal', format = '(e11.4)'
...
;   window, 0, xs = 512, ys = 512, retain = 2
   ...
   device, set_resolution=[512,512], decomposed=0, set_colors=256, $
           z_buffering=0
   erase
   tv, bytscl ((crop_img + cbias)^exp, min = mini, max = maxi)
