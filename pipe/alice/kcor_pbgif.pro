...
; kcor_pbgif.pro
; ------------------------------------------------------------------------------
; derived from fakekcocal16.pro: Written by Alice Lecinski 2013
;
; ------------------------------------------------------------------------------
PRO kcor_pbgif

see_data = 0		; whether or not to display the data processing steps
...
;--- Set up memory for the header and the data.
    ...
    begin ;{
     ...
     ;print, lfilenm
      llen = strlen (lfilenm)
      filenm = strmid (lfilenm, llen-24, 24)	; Remove any directory path.
     ;print, filenm
      gfilenm = strmid (filenm, 0, 20) + '.gif'	; GIF file name
      ...
      line = 14
      hspt = line*len
      datatype = string (hdr (hspt:hspt+79))
      scispot = strpos (datatype, 'science')

      ;--- Engineering data.

      if (scispot ne 24 ) then $
      begin 			;{ We've got engineering data! :-)
        is_eng  = 1
        gfilenm = strmid (filenm, 0, 20) + '_e.gif'
        scispot = 24	; now set scispot to 24 so eng data gets processed too.
      endif ;}

      ;--- Science data.

      if (scispot eq 24 ) then $
      begin			;{ We've got science data! :-)
        iimg = iimg + 1
       ...
       ;print, cavg, ' cavg should be < ', cldlimit, ' to be cloud free'
        ...
        if (( cavg le 1.0 ) or ( cavg ge cldlimit )) then $
	begin ;{
            if ( is_eng eq 0 ) then $
	    begin ;{
            ...
            ;--- Set cavg to an acceptable value so that
	    ;    the data still gets processed below.

            cavg = cldlimit -1
        endif ;}

        ;--- Proceed with processing.  Otherwise, quit.

        if (( cavg ge 1.0 ) and ( cavg le cldlimit )) then $
	begin ;{
	    ...
	    ;
	    ;--- Only find disc center for first image.
	    ...
	    begin ;{
           ...
           ;--- Save central region of image (704x704 pixels).
	   ;--- Resize image: 704x704 -> 512x512 pixels.
           ...
           ;smlrpB0 = rpB0(256:1023-256, 256:1023-256)
            ...
            ;--- Scale image by power of 0.9.

            mypow = 0.9
            smlrpB0 = smlrpB0^mypow
            mxi     = mxi^mypow

	    ;--- Display image on screen (X-window device).
		...
		begin;{
                ...
                set_plot, 'x'
             endif;} end: make gif image
          endif;} end: passed cloud test
       endif;} end: science mode
    end;} end: fits file loop.
    close, 13
end
