...
; 23 Sep 2015 GIF files go to q_path directory, instead of q_dir directory.
;-------------------------------------------------------------------------------
; :Params:
...
PRO kcor_qsc, date, list=list, append=append, gif=gif, base_dir=base_dir
...
IF (np EQ 0) THEN $
BEGIN ;{
   PRINT, "kcor_qsc, 'yyyymmdd', list='list'"
   RETURN
END   ;}
...
log_file  = date + '_kcor_qsc_' + listfile + '.log'
...
;--- quality sub-directories.

q_dir_ok  = q_path + q_ok +  '/'			; ok           images.
q_dir_bad = q_path + q_bad + '/'			; bad          images.
...
;q_dir_unk = q_path + q_unk + '/'			; unknown      images.
;q_dir_eng = q_path + q_eng + '/'			; engineering  images.
;q_dir_ugy = q_path + q_ugy + '/'			; ugly         images.
...
PRINT, "kcor_qsc, '", date, "', list='", list, "'"
;PRINT, 'start_dir:    ', start_dir
PRINT, 'base_dir:     ', base_dir

PRINTF, ULOG, "kcor_qsc, '", date, "', list='", list, "'"
...
;IF (ndim EQ 0) THEN FILE_DELETE, dim_qpath ELSE PRINTF, ULOG, 'ndim: ', ndim
