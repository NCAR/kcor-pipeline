...
;  3 Mar 2015 [GdT] made cal_dir and calfile keywords and set defaults
...
;  6 Mar 2015 [JB] Replaced application of demodulation matrix multiplication with
;                  M. Galloy C-code method. 
;            ***** To execute Galloy C-code you need a new environmental variable in your 
;                  .login or .cshrc:   IDL_DLM_PATH=/hao/acos/sw/idl/kcor/pipe:"<IDL_DEFAULT>"
;-------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
; SYNTAX :
; kcorl1_quick, date_string, list='list_of_L0_files', $
;               base_dir='base_directory_path'
; kcorl1_quick, 'yyyymmdd', list='L0_list', $
;               base_dir='/hao/mlsodata1/Data/KCor/raw'
; 
; EXAMPLES:
; kcorl1_quick, '20141101', list='list17'
; kcorl1_quick, '20141101', list='list17', /append
; kcorl1_quick, '20140731', list='doy212.ls',base_dir='/hao/mlsodata1/Data/KCor/work'
;
...
pro kcorl1_quick, date_str, list=list, base_dir=base_dir, append=append, $
                  cal_dir=cal_dir, calfile=calfile, $
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT: phases are now keywords

default, phase0,  !pi/11.       ; camera 0   16. degrees look ok for 18 Jun 2014 and April 27, 2014
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GdT: calfile and caldir are  now a keyword

default, calfile,  '20150101_190612_kcor_cal_1.0ms.ncdf'
...
;PRINT,        '--- kcorl1_quick ', date_str, ' ---', current_time
;PRINT,        'l0_dir: ', l0_dir
;PRINT,        'l1_dir: ', l1_dir

PRINTF, ULOG, '--- kcorl1_quick ', date_str, ' --- ', current_time
...
;  IF (TYPE EQ 'science') THEN BEGIN
   ...
   sol_ra = sol_ra*15.			; Convert from hours to degrees.
...
;   PRINTF, ULOG, 'occulter size [arcsec] : ', occulter
   ...
   ; Find center and readius for camera 1.
   ...
   pick1 = where (rr1 ge radius_1-1. and rr1 lt 506.)
   mask_occulter1 = fltarr (xsize, ysize)
   mask_occulter1 (*) = 0
   mask_occulter1 (pick1) = 1.

   PRINTF, ULOG, 'CAMERA CENTER INFO FOR RAW MAGES'
...
;-------------------------------------------------------------------------------
   ...
   FOR b = 0, 1 DO BEGIN 
      FOR s = 0, 3 DO BEGIN 
         img (*, *, s, b) = $
            (img (*, *, s, b) - dark_alfred (*, *, b)) / gain_alfred (*, *, b)
      ENDFOR 
   ENDFOR 
...
;   OLD WAY TO APPLY DMAT. REPLACED WITH M. GALLOY CODE BELOW 
...
;   r_out = r_out * radsun * 60. / 5.643
   ...
   r_in  = r_in * radsun  / 5.643
   r_out = r_out * radsun / 5.643
...
;   radsun = ephem (2)
   ...
   ; NOTE: pb0r returns solar radius in arcmin.  sun.pro returns arcseconds.
...
;   PRINTF, ULOG, 'date, hour, minute, second : ', $
;                  date, ' ', hour, ' ',  minute, ' ', second
;   PRINTF, ULOG, 'occulter size [arcsec] : ', occulter
...
;   PRINTF, ULOG, 'Radius of photosphere [pixels] : ', r_photo
   ...
   ;----------------------------------------------------------------------------
   ; Flip.  Rotate for P-angle to put solar north up.
   ;----------------------------------------------------------------------------
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ...
   mini = 0.00	; minimum intensity scaling value.

;   maxi = 1.2	; maximum intensity scaling value.  Used < 28 jan 2015.
;   exp  = 0.8	; scaling exponent.                 Used < 28 Jan 2015.

   maxi = 1.8	; maximum intensity scaling value.  Used > 28 Jan 2015.
   exp  = 0.7   ; scaling exponent.                 Used > 28 Jan 2015.
...
;                        ' calibration file:dark, opal, 4 pol.states'
   sxaddpar, newheader, 'L1SWID',   'kcorl1_quick.pro 04mar2015', $
...
PRINT,        '>>>>>>> End of kcorl1_quick <<<<<<<'
PRINTF, ULOG, '>>>>>>> End of kcorl1_quick <<<<<<<'
PRINTF, ULOG, '- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -'
